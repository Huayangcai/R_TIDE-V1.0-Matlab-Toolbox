function [nameu,fu,yout,st,ft,Eta,Phi,percent,si,cof]=R_tide(xin,Q,t,lat,ray,synth,Qc,twin,sname,IPso,ipre);
...
crit=0.1; % this mathod refers to Ns_tide
...
%           fum(m)=nan;
%           m1=find(snr(m)>synth);%select typical constituents which snr>10
%           if ~isempty(m1)
%              xm=max(tidecon(m,1));i2=find(tidecon(m,1)/xm>0.01&snr(m)>synth);
%               fum(m(i2))=fu(m(i2));
%           else
%              [~,m0] =max(tidecon(m,1));
%              fum(m(m0))=fu(m(m0));
...
if IPso==3
  [cof,stmin]=fmin_Q_tide(xin,Q,t,fu,fband,f,vu,Qc); %it would some time to optimize,per is R2
  cof=cof';
...
if IPso==1
    id=2;
  [cof,per,fband]=Rtide_pso(xin,Q,t,fu,id,vu,f,Qc,fband); %it would some time to optimize,per is R2
 if time(1,1)==time(end,1)
