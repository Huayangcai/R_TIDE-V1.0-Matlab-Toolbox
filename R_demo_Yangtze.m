...
mm=find(t==datenum(2009,1,1,0,0,0));
ii1=1:mm-1; % select the time series for calibration （2002-2008）
T1=t(ii1,1); % time series from 202 to 2008
Q1=Q(ii1,1); % river discharge from 202 to 2008
Qc1=max(Q1); % the default value of critical discharge
ray=1; % Rayleigh criteria
synth=10; % signal-to-noise ratios
twin=366; % days for window spectrum
ipso=1; 
...
% If ipre=2, directly predict. Divide the data series into 2 parts, one of which is used to derive coefficient, another one is used to predict.
% Users should set the length of data series in Rtide_pre.m.

lat=[32.04044;31.9260;32.2293;32.0518;31.7026;31.3291]; % latitude of the tidal gauging selected station
...
% This program does not provide more options such as the solutions to system of linear algebraic equations, and do not add analysis of inference tide
...
% harmonic analysis
for i=1:size(z,2)
    xin1=z(ii1,i); % read the water level of selected time series in each tidal gauging station
    sname=char(stname(i));% read the name of tidal gauging station
    lat1=lat(i); %  read the latitude of tidal gauging station
    dH=0.001;% The critical tidal range, if dH=0.001, the corresponding discharge is the critical discharge Qc. If the discharge exceeds this value, there is no tidal signal in this station.    
    ...
    N=1;
    n=length(fu);% three major tidal constituents 
    M1=length(Q1);
...
% compute the time-dependent amplitudes and phases of n tidal constituents
  ...
  for kk=1:n  %the number of constituents
...
jj=0;
 ...
 % compute variance s(t)_zr and f(t)_ztr through hourly data and daily averaged data
  varhour(k,1)=var(y); 
  ...
  clear z1 u1 timet1 x11 x21 x31 x41 x51 x61
    ...
    save (fname2,'z','y2','Eta','Phi','ab','cof','b1','percent','st','ft','QQ1')
    %% validation
    % In this part, we use the coefficients derived from calibration period
    % as well as the river discharge records in the rest time to
 ...
 ii2=mm:size(z,1); % data series from 2009 to 2012
    xin2=z(ii2,i);% water level from 2009 to 2012
    T2=t(ii2,1);% time series from 2009 to 2012
    Q2=Q(ii2,1); % river discharge record from 2009 to 2012
    M2=length(Q2);
    ...
    for j=1:n
     ...
     clear j
       ...
       YY2(iz2,1)=yy2; %regression water level
